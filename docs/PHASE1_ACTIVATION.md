...
Total **12/12 bounded synthetic calls**, zero retries, and zero 4/4 candidates. No repeatability run was warranted; `WRITER_MODEL` was **not** set. All currently active Preview text routes still use the stable Nemotron Super baseline; this does not make it an accepted Indonesian surface Writer. The exact-commit paid toggle was set to `disabled` after the last diagnostic. Production remains Gateway. **Phase 1 stays INCOMPLETE; do not begin Phase 2.** A proposed Plan B is an evidence-grounded structured scene plan from Nemotron, a constrained surface Writer, and a deterministic fact/length check before Guardian review; this is inferred from failures, has **not** been implemented or accepted, and requires a separate decision. No more blind candidate sampling under this shortlist.
...
Regression on this branch after the role-specific change and benchmarks: **24/24 Node tests pass**, non-incremental TypeScript check passes, clean local Next build passes, and local signed-out HTTP Gateway/Nebius AI/Memory/history smoke passes. The last READY Preview before these intentionally failed evaluation builds is `dpl_9FtztSg49XEUHw6YiiZwgWhYhrzY` (`9fee270`), with the baseline Super runtime. A read-only Production `/api/ai` check returned HTTP 200, `provider=gateway`, `verification=configuration-only`; no production promotion occurred. The local regressions do not turn failed prose samples into acceptance.
