...
## Latest checkpoint: Plan C corpus preparation — 2026-09-24 UTC

Plan B was implemented as a bounded experimental adapter, then **failed** the four-case Indonesian prose gate; its full first-pass/repair evidence is in [`qa/phase1-planb-2026-09-24.md`](../qa/phase1-planb-2026-09-24.md). It was not routed into the active application. Plan C now has an **original synthetic 120-train/30-held-out corpus and validator**, described in [`qa/plan-c/README.md`](../qa/plan-c/README.md). No private novel was read, no Nebius file was uploaded, no job was created, and no model or Production setting changed. The latest official Nebius fine-tuning documentation lists the exact three proposed LoRA IDs; authenticated model-specific LoRA rates and Dedicated serving costs are still unavailable from the signed-out console. The one-epoch training cost **cannot yet be quoted exactly**. Finish authenticated pricing and a full human reference-text audit before requesting approval for even a small paid pilot. **Phase 1 remains INCOMPLETE; Phase 2 blocked; Production Gateway.**
