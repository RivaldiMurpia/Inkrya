...
## Latest checkpoint: Plan C `gpt-oss-20b` training cost gate — 2026-09-24 UTC

Plan B was implemented as a bounded experimental adapter, then **failed** the four-case Indonesian prose gate; its full first-pass/repair evidence is in [`qa/phase1-planb-2026-09-24.md`](../qa/phase1-planb-2026-09-24.md). It was not routed into the active application. Plan C now has an **original synthetic 120-train/30-held-out corpus and validator**, described in [`qa/plan-c/README.md`](../qa/plan-c/README.md). The account's authenticated SFT console offered `gpt-oss-20b` LoRA at **$2.00 / 1M tokens for 8K**, with a **$25.00 balance and $0.97 trial credit** at inspection. The exact Nebius job base ID is `unsloth/gpt-oss-20b-BF16`. The checked base-model tokenizer and chat template measure **52,877 train tokens** and **13,206 sealed held-out tokens**; proposed **one epoch, no validation file** has a **$0.105754 token-price component** before tax, minimum/packing differences or other charges. The console configuration and limitations are recorded in the pilot plan. No published minimum was found in the current SFT/dataset/billing docs; absence of one is **unconfirmed**, and a visible balance is not a spending cap. A full human audit of all reference texts remains before any upload or paid pilot. No private novel was read, no Nebius file was uploaded, no job was created, and no model or Production setting changed. **Phase 1 remains INCOMPLETE; Phase 2 blocked; Production Gateway.**
