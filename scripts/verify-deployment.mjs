...
// inference. Explicit modes smoke/prose opt into bounded paid calls.
...
if (!process.exitCode && /^[a-f0-9]{40}$/.test(smokeCommit||'') && smokeCommit===process.env.VERCEL_GIT_COMMIT_SHA) {
  if(process.env.VERCEL_ENV!=='preview'||process.env.VERCEL_GIT_COMMIT_REF!=='hackathon/nebius-2026') {
    ...
    const script=fileURLToPath(new URL(mode==='smoke'?'./phase1-smoke.mjs':mode==='prose'?'./phase1-prose.mjs':'./phase1-readback.mjs',import.meta.url));
