...
export async function POST(req:Request){
 ...
 try{
  const result=await generateKryaText(prepared,{maxOutputTokens:outputTokensForWriting(action,instruction,prepared.config.provider==='nebius'&&prepared.config.id==='nvidia/nemotron-3-super-120b-a12b'),timeoutMs:35000,system:systemForAction(action),prompt:JSON.stringify({action,instruction,selected_text:selection,context:bounded.sources,context_truncated:bounded.truncated})},{generationId:run.id,projectId,userId:auth.user.id,sourceCount:bounded.sources.length,workflow:'krya-assistant'});
